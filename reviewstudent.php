...
<head>
    ...
    <title>Display Teacher Feedback</title>
    <link
...
<table class="table table-striped">
  <thead>
    <tr>
      ...
      <th  class="table-dark" scope="col">Student Name</th>
...
<tbody>
  ...
  $sql = "SELECT * FROM stufeedback  WHERE email='$smail'";
  $result = $conn->query($sql);

  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      $num++;
      $modalId = "feedbackModal".$num; // Define unique modal ID for each row
  ?>
    <tr>
